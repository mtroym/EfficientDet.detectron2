...
from detectron2.utils.logger import log_first_n, setup_logger
...
def permute_all_cls_and_box_to_N_HWA_K_and_concat(box_cls, box_delta, num_classes=80):
    ...
    """
    # logging.getLogger(__name__).info([x.shape for x in box_cls])
    # logging.getLogger(__name__).info([x.shape for x in box_delta])
...
class EfficientDet(nn.Module):
    ...
    def losses(self, gt_classes, gt_anchors_deltas, pred_class_logits, pred_anchor_deltas):
        ...
        # logging.getLogger(__name__).info(loss_box_reg.shape, pred_anchor_deltas.shape)
